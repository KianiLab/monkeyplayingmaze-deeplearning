...
# # modify the last classifer（classifier‘s 6th layer），将其输出特征数从1000改为4
...
class CustomAlexNet(nn.Module):
    def __init__(self, num_traj_features, num_classes):
        ...
        # self.dropout=nn.Dropout(0.5)
        # # Adjust the first Linear layer to accept additional trajectory features along with the original features
        # self.alexnet.classifier[1] = nn.Linear(num_ftrs + num_traj_features, 4096)
        
     
        # # modify the last classifer（classifier‘s 6th layer），将其输出特征数从1000改为4
        # self.alexnet.classifier[6] = nn.Linear(in_features=self.alexnet.classifier[6].in_features, out_features=num_classes)
        # # Batch normalization for fully connected layers
        # self.batch_norm_class = nn.BatchNorm1d(in_features=self.alexnet.classifier[6].in_features)
...
def run_training_loop(config, model, train_loader, val_loader,device, optimizer, criterion,scheduler, early_stopper,logger):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            save_path=os.path.join(model_path,"cnn2.pth")
            torch.save(model.state_dict(), save_path)
       
              # Save to W&B directory
            wandb_save_path = os.path.join(wandb.run.dir, "model_best.pth")
            torch.save(model.state_dict(), wandb_save_path)
...
# Optionally normalize the weights
    ...
    # If the validation loss does not improve after this many epochs, the training will be stopped.
    # early_stopper = EarlyStopper(patience=10, min_delta=0.01)
    history=run_training_loop(config, model, train_loader, val_loader, device, optimizer, criterion,scheduler,early_stopper,logger)
